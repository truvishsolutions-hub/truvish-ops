...
const Theme = ({ value = null, onChange }) => {
    ...
    useEffect(() => {
        const loadThemes = async () => {
            try {
                ...
                const themeList = [];
                const imageSlots = [1, 2, 3, 4, 6, 7, 8, 9, 11, 12, 13, 14, 16, 17, 18, 19];

                imageSlots.forEach((slot) => {
                    const image = config[`img${slot}`];
                    const name = config[`img${slot}Name`];

                    if (image && String(image).trim() !== "" && name && String(name).trim() !== "") {
                        themeList.push({
                            id: String(slot),
    ...
    const handleThemeChange = (event) => {
        ...
        const selected = themes.find((theme) => String(theme.id) === String(selectedId));

        if (!selected) {
            console.warn("Selected theme not found:", selectedId);
            return;
        }
    ...
    return (
        <div className="t-mini-card">
            ...
            {loading && <div className="t-theme-status">Loading themes...</div>}

            {!loading && error && <div className="t-theme-status t-theme-error">{error}</div>}

            {!loading && !error && themes.length === 0 && (
                <div className="t-theme-status t-theme-error">No themes available.</div>
            )}
            ...
            {!loading && selectedTheme && themes.length > 0 && (
                (() => {
                    const selected = themes.find((item) => String(item.id) === String(selectedTheme));
                    ...
                    return (
                        <div className="t-theme-preview">
                            <img
                                ...
                                alt={selected.name}
                                onError={(e) => {
                                    console.warn("Theme image failed:", selected.image);
            ...
            <div className="t-theme-info">
                ...
                <span>Theme will be applied to all codes in this order.</span>
            </div>
        </div>
    );
};
